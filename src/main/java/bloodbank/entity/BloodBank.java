...
@NamedQueries({
	...
	@NamedQuery( name = BloodBank.DONATION_COUNT, query = "SELECT b FROM BloodBank b WHERE size(b.donations)>= 0"),
	@NamedQuery(name = BloodBank.BLOODBANK_BY_ID_QUERY, query = "SELECT b FROM BloodBank b WHERE b.id = :param1")
})
public abstract class BloodBank extends PojoBase implements Serializable {
	...
	public static final String BLOODBANK_BY_ID_QUERY = "BloodBank.bloodBankById";
